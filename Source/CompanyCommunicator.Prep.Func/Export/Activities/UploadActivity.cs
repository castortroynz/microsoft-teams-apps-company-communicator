                ...
                using (var entryStream = messageDeliveryFile.Open())
                {
                    ...
                    using (var csv = new CsvWriter(writer, CultureInfo.InvariantCulture))
                    {
                        if (uploadData.sentNotificationDataEntity.Teams.Any())
                        {
                            ...
                            var teamDataStream = this.userDataStream.GetTeamDataStreamAsync(uploadData.sentNotificationDataEntity.NotificationId, uploadData.sentNotificationDataEntity.Status);
                        ...
                        else
                        {
                            ...
                            var userDataStream = this.userDataStream.GetUserDataStreamAsync(uploadData.sentNotificationDataEntity.NotificationId, uploadData.sentNotificationDataEntity.Status);
